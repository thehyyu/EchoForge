...
### 輸入來源 A：語音錄音
錄音上傳 → Whisper large-v3 轉逐字稿（Mac mini）→ 等待人工審核
→ 後台顯示逐字稿（可編輯）+ Prompt（可調整）
→ 人工點「產生草稿」→ Vercel API 寫入 `job(type='generate')` → poll.py（Mac mini）取得任務
→ Qwen2.5 潤飾中文 + 自動分類 + 萃取關鍵字 → 寫回草稿 → 後台顯示結果

### 輸入來源 B：Gemini 對話
貼上 Gemini share URL → Mac mini 爬取對話內容
→ Qwen2.5（Ollama）整理成中文文章 + 分類 + 關鍵字 → 存為草稿

### 共用後段
草稿 → 人工校稿（可分別編輯中英版本）→ 發佈
→ Vercel API 寫入 `job(type='translate')` → poll.py（Mac mini）取得任務
→ Qwen2.5 翻譯英文 + 萃取 tags_en → 寫回 posts
...
### Ollama 隔離原則
Ollama（Qwen2.5）永遠只在 Mac mini 本機執行，不對外暴露。
Vercel 上的 API route **不得**直接呼叫 Ollama。
所有需要 AI 處理的動作，一律透過寫入 `jobs` 資料表，
...
**Input:** DB 連線正常
**Output:** jobs 資料表
      （id, type, source_url, transcript, post_id, status, error_message, created_at）
**Success criteria:**
- [ ] [RED] 欄位不存在時查詢應失敗
- [ ] [GREEN] Migration 執行後所有欄位存在
- [ ] type 欄位限制只接受 voice / gemini / generate / translate
  - voice：語音轉逐字稿
  - gemini：Gemini 對話爬取
  - generate：從逐字稿產生草稿（Qwen2.5）
  - translate：翻譯已發佈文章（Qwen2.5）
- [ ] status 流程：pending → processing → transcribed → done（或 error）
  - transcribed：Whisper 完成，等待人工審核
  - done：草稿已產生 / 翻譯已完成
...
**Output:** 逐字稿字串，存入 jobs.transcript，status 改為 transcribed
**Success criteria:**
- [ ] [RED] 給定測試音檔，函式應回傳非空字串
- [ ] [GREEN] Whisper 呼叫函式實作完成
- [ ] 轉錄結果存回 jobs.transcript，status 改為 transcribed（停下等人工）
...
**Input:** Branch 5.4、5.5 已完成
**Output:** 所有 Ollama 呼叫集中在 poll.py，Vercel 不直接呼叫 Ollama
**Success criteria:**
- [ ] `generate/route.ts` 改為寫入 `job(type='generate')`，不呼叫 Ollama
- [ ] `publish/route.ts` 改為寫入 `job(type='translate')`，不呼叫 Ollama
- [ ] DB migration：jobs.type 擴充接受 `generate` / `translate`
- [ ] poll.py 新增 `generate` job 處理邏輯（Qwen2.5 潤飾 + 分類 + 關鍵字 → 寫入 posts）
- [ ] poll.py 新增 `translate` job 處理邏輯（Qwen2.5 翻譯 + tags_en → 更新 posts）
- [ ] 後台草稿頁輪詢 job status，完成後顯示草稿
- [ ] pytest 覆蓋 generate / translate job 處理邏輯

### 6.3 上線

**Input:** 所有測試通過、Vercel 部署正常、Branch 6.4 完成
**Output:** 線上可存取的 blog
**Success criteria:**
- [ ] 自訂網域設定完成（optional）
- [ ] HTTPS 正常
