...
export default async function SearchPage({
  ...
  let posts: { slug: string; title_zh: string; category: string; created_at: string }[] = []
          ...
          {posts.map((post) => (
            <li key={post.slug}>
              <p className="text-xs text-gray-400 mb-1">
                {post.category} · {new Date(post.created_at).toLocaleDateString('zh-TW')}
              </p>
              <Link href={`/zh/posts/${post.slug}`} className="text-lg font-semibold hover:underline">
                {post.title_zh}
              </Link>
            </li>
          ))}
